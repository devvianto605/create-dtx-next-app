...
export const renderVersionWarning = (npmVersion: string) => {
  ...
  if (currentVersion.includes("beta")) {
    logger.warn("  You are using a beta version of create-dtx-next-app.");
    logger.warn("  Please report any bugs you encounter.");
  } else if (currentVersion.includes("next")) {
    logger.warn(
      "  You are running create-dtx-next-app with the @next tag which is no longer maintained."
    );
    logger.warn("  Please run the CLI with @latest instead.");
  } else if (currentVersion !== npmVersion) {
    logger.warn("  You are using an outdated version of create-dtx-next-app.");
    logger.warn(
      "  Your version:",
      currentVersion + ".",
...
function checkForLatestVersion(): Promise<string> {
  return new Promise((resolve, reject) => {
    https
      .get(
        "https://registry.npmjs.org/-/package/create-dtx-next-app/dist-tags",
        (res) => {
...
export const getNpmVersion = () =>
  ...
  checkForLatestVersion().catch(() => {
    try {
      return execSync("npm view create-dtx-next-app version").toString().trim();
    } catch {
      return null;
    }
  });
